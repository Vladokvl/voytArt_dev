...
export async function createPaintingInquiryAction(input: CreateInquiryInput) {
  try {
    ...
    // Надійний номер запиту: 4 байти CSPRNG (~4.3 млрд варіантів, замість 65536
    // при 2 байтах) + retry на колізію unique-поля
    let inquiry: { inquiryNumber: string; id: number } | null = null;
    for (let attempt = 0; attempt < 3 && !inquiry; attempt++) {
      const inquiryNumber = `#INQ-${crypto.randomBytes(4).toString("hex").toUpperCase()}`;
      try {
