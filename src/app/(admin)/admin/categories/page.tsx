...
export default async function CategoriesPage({
  ...
  const { sortBy = "name", sortDir = "asc", error } = await searchParams;
  const validSortDir: "asc" | "desc" = sortDir === "desc" ? "desc" : "asc";

  let orderByQuery: Prisma.CategoryOrderByWithRelationInput = { name: validSortDir };
  if (sortBy === "name") {
  ...
  return (
    <div>
      {/* Повідомлення про помилку видалення (з deleteCategoryAction) */}
      {error === "products-linked" && (
        <div
          style={{
            background: "#fef2f2",
            color: "#b91c1c",
          ...
          role="alert"
        >
          Неможливо видалити категорію, оскільки вона містить товари. Спочатку перенесіть їх в іншу категорію.
        </div>
      )}
      {/* Header */}
