...
const RangeSlider = (props: RangeSliderProps) => {
  ...
  const handleTrackMouseDownCapture = (
    ...
    const _nearestHandle = direction === 'ltr' ? nearestHandle : nearestHandle === 1 ? 0 : 1
    ...
    if (!disabled) {
      switch (event.key) {
        ...
        case 'ArrowRight': {
          ...
          const nextValue =
            stepOnMarks && marks
              ? (direction === 'rtl' ? getPreviousMarkValue : getNextMarkValue)(
        ...
        case 'ArrowLeft': {
          ...
          const nextValue =
            stepOnMarks && marks
              ? (direction === 'rtl' ? getNextMarkValue : getPreviousMarkValue)(
                  valueRef.current[focusedIndex],
                  marks
                )
              : Math.min(
  ...
  return (
    <div {...rest} ref={ref}>
      ...
      <input type="hidden" name={`${name}-from`} value={_value[0]} {...inputProps} />
      <input type="hidden" name={`${name}-to`} value={_value[1]} {...inputProps} />
    </div>
  )
}
