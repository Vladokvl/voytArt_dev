...
const Slider = (props: SliderProps) => {
  ...
  const [_value, setValue] = useControllableState({
    ...
    defaultProp: typeof defaultValue === 'number' ? clamp(defaultValue, min!, max!) : defaultValue,
    onChange,
  })
  ...
  const _tooltip = typeof tooltip === 'function' ? tooltip(scaledValue) : tooltip
  ...
  const handleTrackKeydownCapture = (event: KeyboardEvent<HTMLDivElement>) => {
    if (!disabled) {
      switch (event.key) {
        case 'ArrowUp': {
          ...
          if (stepOnMarks && marks) {
            const nextValue = getNextMarkValue(_value as number, marks)
            setValue(nextValue)
          ...
          const nextValue = getFloatingValue(
            Math.min(Math.max((_value as number) + step!, min!), max!),
            precision
          )
          setValue(nextValue)
          onDraggingStop?.(nextValue)
        ...
        case 'ArrowRight': {
          ...
          const nextValue = getFloatingValue(
            Math.min(
              Math.max(
                direction === 'rtl' ? (_value as number) - step! : (_value as number) + step!,
                min!
              ),
              max!
            ),
        ...
        case 'ArrowDown': {
          ...
          if (stepOnMarks && marks) {
            const nextValue = getPreviousMarkValue(_value as number, marks)
            setValue(nextValue)
            onDraggingStop?.(nextValue)
            break
          }
