...
function AutoComplete<T>(props: AutoCompleteProps<T>) {
  ...
  useEffect(() => {
    const items = data.filter(item => optionKey(item).toLowerCase().includes(value.toLowerCase()))
    setOptions(items)
  ...
  return (
    <>
      ...
      <FloatingPortal>
        {open && (
          <FloatingFocusManager visuallyHiddenDismiss context={context} initialFocus={-1}>
            <div
              {...getFloatingProps({
