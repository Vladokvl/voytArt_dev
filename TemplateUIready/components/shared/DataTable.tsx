...
// Extended ColumnDef with sortable property
...
function DataTable<T>(props: DataTableProps<T>) {
  ...
  const handlePaginationChange = useCallback(
    ...
    [loading, resetSelected, onPaginationChange]
  )
  ...
  // Мемоізуємо значення для Select
  const selectedPageSize = useMemo(
    () => pageSizeOption.filter(option => option.value === pageSize),
    [pageSizeOption, pageSize]
  )

  return (
    <Loading loading={Boolean(loading)} type="cover">
      <Table {...rest}>
        <THead>
                      ...
                      <div
                        className={classNames(
                          header.column.getCanSort() &&
                            (header.column.columnDef as ExtendedColumnDef<T>).sortable &&
                ...
                <Td className="hover:bg-transparent" colSpan={finalColumns.length}>
                  <div className="flex flex-col items-center gap-4">
                    {customNoDataIcon ? (
                      ...
                      <>
                        <FileNotFound />
                        <span className="font-semibold">{t('noDataFound')}</span>
                      </>
                    )}
                  </div>
                </Td>
      ...
      <div className="flex items-center justify-between mt-4">
        ...
        />
        <div className="min-w-[140px] min-h-[2.25rem] flex items-center">
          <Select
