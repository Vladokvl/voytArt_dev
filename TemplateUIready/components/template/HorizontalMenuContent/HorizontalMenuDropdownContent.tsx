...
import type { NavigationTree, HorizontalMenuMeta, TranslationFn } from '@/@types/navigation'
...
const TabLayout = ({
  ...
  return (
    <div className="flex">
      ...
      {navigationTree.some(nav => nav.key === activeKey && nav.type === NAV_ITEM_TYPE_COLLAPSE) && (
  ...
  return (
    ...
    >
      ...
      {layoutMeta?.layout === 'tabs' && (
        <TabLayout navigationTree={navigationTree} columns={layoutMeta.columns} {...rest} />
      )}
      {!layoutMeta && <DefaultLayout navigationTree={navigationTree} {...rest} />}
    </div>
  )
}
